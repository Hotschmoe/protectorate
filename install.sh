...
#   4. Generates long-lived OAuth token
#   5. Clones Protectorate repo (latest release)
#   6. Creates .env configuration
#   7. Pulls pre-built container images
#   8. Starts Envoy
#
set -e
...
generate_token() {
    prompt "Generating long-lived OAuth token (valid 1 year). You'll need to authorize in your browser."

    # Run setup-token and capture output
    TOKEN_OUTPUT=$(claude setup-token 2>&1) || true

    # Extract token from output
    # Expected format: "Your OAuth token (valid for 1 year): sk-ant-oat01-..."
    TOKEN=$(echo "$TOKEN_OUTPUT" | grep -oP 'sk-ant-oat01-[A-Za-z0-9_-]+' | head -1)

    if [[ -z "$TOKEN" ]]; then
        warn "Could not automatically extract token."
        echo ""
        echo "Output was:"
        echo "$TOKEN_OUTPUT"
        echo ""
        echo "Please copy the token (starts with sk-ant-oat01-) and paste it below:"
        read -p "Token: " TOKEN

        if [[ -z "$TOKEN" ]]; then
            warn "No token provided. You can manually run 'claude setup-token' later and add to .env"
            return 1
        fi
    fi
...
prompt_optional_vars() {
    ...
    echo "(Press Enter to skip any)"
    echo ""

    read -p "Anthropic API Key (for direct API access): " ANTHROPIC_KEY
    if [[ -n "$ANTHROPIC_KEY" ]]; then
        echo "ANTHROPIC_API_KEY=$ANTHROPIC_KEY" >> "$ENV_FILE"
    fi

    read -p "Google AI API Key (for Gemini CLI): " GOOGLE_KEY
...
main() {
    ...
    if [[ -z "$TOKEN" ]]; then
        warn "Continuing without long-lived token (will use mounted credentials)"
        TOKEN=""
    fi

    # 4. Get latest release and setup repo
